...
<div>
    ...
    @foreach ($graybeards as $graybeard)
        <div class="modal fade" id="editModal{{ $graybeard->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <form action="{{ $Action }}/save-edit/{{ $graybeard->id }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="recipient-name" class="col-form-label">Email</label>
                                <input type="email" class="form-control" name="email" id="fullname"
                                    value={{ $graybeard->email }}>
                            </div>

                            <div class="mb-3">
                                <label for="recipient-name" class="col-form-label">Fullname</label>
                                <input type="text" class="form-control" name="fullname" id="fullname"
                                    value={{ $graybeard->fullname }}>
                            </div>

                            <div class="mb-3">
                                <label for="recipient-name" class="col-form-label">Password</label>
                                <input type="password" class="form-control" name="password"
                                    value={{ $graybeard->password }}>
                            </div>
            ...
            <div class="table-responsive">
                <table class="table table-lg">
                    ...
                    <tbody>
                        @foreach ($graybeards as $graybeard)
                            <tr>
                                <td class="text-bold-500">{{ $graybeard->id }}</td>
                                <td>{{ $graybeard->email }}</td>
                                <td>{{ $graybeard->fullname }}</td>
                                <td><span class="badge rounded-pill bg-danger">Secret content</span></td>
                                <td>
                                    <a href="#" type="button" class="btn btn-primary mx-2" data-bs-toggle="modal"
                                        data-bs-target="#editModal{{ $graybeard->id }}">
                                    ...
                                    <a href="{{ $Action }}/{{ $graybeard->id }}" class=" mx-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
